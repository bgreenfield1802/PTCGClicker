const packs = {
    swshPromo: {
        name: "Sword & Shield Promos", name2: "SWSH Promo",
        price: 0, cards: 222, total: 222,
        img: "img/swsh_promo.png", cardImg: "https://assets.pokemon.com/assets/cms2/img/cards/web/SWSHP/SWSHP_EN_SWSH"
    },
    swsh1: {
        name: "Sword & Shield Base Set", name2: "Sword & Shield",
        contents: [1,5,3,1,1], // energy, commons, uncommons, reverse, rare
        desc: "Contains 10 cards + 1 energy", imgNum: 1,
        price: 4, cards: 202, secret: 14, total: 216,
        img: "img/swsh1/swsh_base_set", cardImg: "https://assets.pokemon.com/assets/cms2/img/cards/web/SWSH1/SWSH1_EN_"
    },
    swsh2: {
        name: "Sword & Shield Rebel Clash", name2: "Rebel Clash",
        contents: [1,5,3,1,1], // energy, commons, uncommons, reverse, rare
        desc: "Contains 10 cards + 1 energy", imgNum: 1,
        price: 4, cards: 192, secret: 17, total: 209,
        img: "img/swsh2/swsh_rebel_clash", cardImg: "https://assets.pokemon.com/assets/cms2/img/cards/web/SWSH2/SWSH2_EN_"
    },
    swsh3: {
        name: "Sword & Shield Darkness Ablaze", name2: "Darkness Ablaze",
        contents: [1,5,3,1,1], // energy, commons, uncommons, reverse, rare
        desc: "Contains 10 cards + 1 energy", imgNum: 1,
        price: 4, cards: 189, secret: 12, total: 201,
        img: "img/swsh3/swsh_darkness_ablaze", cardImg: ""
    },
    futsalPromo: {
        name: "Pokemon Futsal Promos", name2: "Futsal Promo",
        price: 0, cards: 5, total: 5,
        img: "img/futsal_promo.png", cardImg: ""
    },
    swsh4: {
        name: "Champion's Path", name2: "Champion's Path",
        contents: [0,5,3,1,1], // energy, commons, uncommons, reverse, rare
        desc: "Contains 10 cards", imgNum: 1,
        price: 5, cards: 73, secret: 7, total: 80,
        img: "img/swsh4/swsh_champions_path", cardImg: ""
    },
    swsh5: {
        name: "Sword & Shield Vivid Voltage", name2: "Vivid Voltage",
        contents: [1,5,3,1,1], // energy, commons, uncommons, reverse, rare
        desc: "Contains 10 cards + 1 energy", imgNum: 4,
        price: 4, cards: 185, secret: 18, total: 203,
        img: "img/swsh5/swsh_vivid_voltage", cardImg: ""
    },
    mcd1: {
        name: "McDonald's 25th Anniversary", name2: "McDonald's 25th",
        contents: [0,3,0,1,0], // energy, commons, uncommons, reverse, rare
        desc: "Contains 4 cards", imgNum: 1,
        price: 2, cards: 25, secret: 0, total: 25,
        img: "img/mcd1/mcd_25th_anniversary", cardImg: ""
    },
    swsh6: {
        name: "Shining Fates", name2: "Shining Fates",
        contents: [0,5,3,1,1], // energy, commons, uncommons, reverse, rare
        desc: "Contains 10 cards", imgNum: 4,
        price: 5, cards: 72, secret: 1, subset: 122, total: 73,
        img: "img/swsh6/swsh_shining_fates", cardImg: ""
    },
    swsh7: {
        name: "Sword & Shield Battle Styles", name2: "Battle Styles",
        contents: [1,5,3,1,1], // energy, commons, uncommons, reverse, rare
        desc: "Contains 10 cards + 1 energy", imgNum: 4,
        price: 4, cards: 163, secret: 20, total: 183,
        img: "img/swsh7/swsh_battle_styles", cardImg: ""
    },
    swsh8: {
        name: "Sword & Shield Chilling Reign", name2: "Chilling Reign",
        contents: [1,5,3,1,1], // energy, commons, uncommons, reverse, rare
        desc: "Contains 10 cards + 1 energy", imgNum: 5,
        price: 4, cards: 198, secret: 35, total: 233,
        img: "img/swsh8/swsh_chilling_reign", cardImg: ""
    },
    swsh9: {
        name: "Sword & Shield Evolving Skies", name2: "Evolving Skies",
        contents: [1,5,3,1,1], // energy, commons, uncommons, reverse, rare
        desc: "Contains 10 cards + 1 energy", imgNum: 4,
        price: 4, cards: 203, secret: 34, total: 237,
        img: "img/swsh9/swsh_evolving_skies", cardImg: ""
    },
    cele1: {
        name: "Pokemon 25th Celebrations", name2: "25th Celebrations",
        contents: [0,0,0,0,4], // energy, commons, uncommons, reverse, rare
        desc: "Contains 4 cards", imgNum: 1,
        price: 2, cards: 25, secret: 0, subset: 25, total: 25,
        img: "img/cele1/swsh_25th_celebrations", cardImg: ""
    },
    swsh10: {
        name: "Sword & Shield Fusion Strike", name2: "Fusion Strike",
        contents: [1,5,3,1,1], // energy, commons, uncommons, reverse, rare
        desc: "Contains 10 cards + 1 energy", imgNum: 4,
        price: 4, cards: 264, secret: 20, total: 284,
        img: "img/swsh10/swsh_fusion_strike", cardImg: ""
    },
    swsh11: {
        name: "Sword & Shield Brilliant Stars", name2: "Brilliant Stars",
        contents: [1,5,3,1,1], // energy, commons, uncommons, reverse, rare
        desc: "Contains 10 cards + 1 energy", imgNum: 4,
        price: 4, cards: 172, secret: 15, subset: 30, total: 187,
        img: "img/swsh11/swsh_brilliant_stars", cardImg: ""
    }
}